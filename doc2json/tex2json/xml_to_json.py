...
from s2orc_doc2json.doc2json.utils.grobid_util import (
    parse_bib_entry,
    get_author_data_from_grobid_xml,
)
from s2orc_doc2json.doc2json.s2orc import Paper, Paragraph


SKIP_TAGS = {"clearpage", "colorpool", "newpage", "tableofcontents"}

TEXT_TAGS = {"p", "proof", "caption"}
...
def process_author(
    author_text: str, grobid_client: GrobidClient, logfile: str
) -> List[Dict]:
    """
    Process authors
    :param author_text:
...
def process_paragraph(
    ...
    all_eq_spans = []
    for span in itertools.chain(
        re.finditer(r"(INLINEFORM\d+)", text), re.finditer(r"(DISPLAYFORM\d+)", text)
    ):
        try:
...
def process_metadata(
    sp: BeautifulSoup, grobid_client: GrobidClient, log_file: str
) -> Tuple[str, List]:
    """
                    ...
                    authors.append(
                        {
                            ...
                            "last": author_parts[-1]
                            if author_parts[-1].lower()
                            not in {"jr", "jr.", "iii", "iv", "v"}
                            else author_parts[-2]
                ...
                try:
                    ...
                    if bib_par.text:
                        ...
                        if not next_tag.find("bibitem") and next_tag.text:
                            bib_entry = process_bibentry(
                                next_tag.text, client, log_file
                            )
                        else:
                            bib_entry = None
                ...
                try:
                    ...
                    if bib_name:
                        ...
                        if bib_name:
                            ...
                            bib_entry = process_bibentry(
                                bib_name.group(2), client, log_file
                            )
                    else:
                        bib_lines = bib_text.split("\n")
...
def get_sections_from_div(
    el: bs4.element.Tag, sp: BeautifulSoup, parent: Optional[str], faux_max: int
) -> Dict:
    """
    ...
    for sub_el in el.find_all(recursive=False):
        if sub_el.name.startswith("div"):
            ...
            sec_keys = [
                int(k.strip("SECREF"))
                for k in sec_map_dict.keys()
                if k and k.strip("SECREF").isdigit()
            ]
            faux_max = max(sec_keys + [faux_max]) + 1
            sec_map_dict.update(
                get_sections_from_div(
                    sub_el, sp, el_ref_id if el_ref_id else parent, faux_max
                )
            )
                    ...
                    if fig.get("file") and fig.get("extension"):
                        ...
                        for subfig in fig.find_all("subfigure"):
                            if subfig.get("file") and subfig.get("extension"):
                                fig_files.append(
                                    subfig.get("file") + "." + subfig.get("extension")
                                )
        ...
        try:
            if flt.name and flt.get("name") == "table":
                if flt.get("id"):
                    ...
                    html = (
                        convert_table_to_html(content) if keep_table_contents else None
                    )
                    # form tabmap entry
                    table_map[ref_id] = {
                        "num": flt.get("id-text", None),
...
def combine_ref_maps(
    eq_map: Dict, fig_map: Dict, tab_map: Dict, foot_map: Dict, sec_map: Dict
):
    """
...
def process_abstract_from_tex(
    sp: BeautifulSoup, bib_map: Dict, ref_map: Dict
) -> List[Dict]:
    """
    ...
    if sp.abstract:
        ...
        if sp.std:
            p_tags = [
                tag
                for tag in sp.std
                if tag.name == "p" and not tag.get("s2orc_id", None)
            ]
        elif sp.unknown:
            p_tags = [
                tag
                for tag in sp.unknown
                if tag.name == "p" and not tag.get("s2orc_id", None)
            ]
        else:
            p_tags = None
...
def get_seclist_for_el(
    el: bs4.element.Tag, ref_map: Dict, default_seclist: List
) -> List[Tuple]:
    """
...
def process_div(
    tag: bs4.element.Tag, secs: List, sp: BeautifulSoup, bib_map: Dict, ref_map: Dict
) -> List[Dict]:
    """
...
def process_body_text_from_tex(
    sp: BeautifulSoup, bib_map: Dict, ref_map: Dict
) -> List[Dict]:
    """
...
def convert_xml_to_s2orc(
    ...
    refkey_map = combine_ref_maps(
        equation_map, figure_map, table_map, footnote_map, section_map
    )
...
def convert_latex_xml_to_s2orc_json(
    xml_fpath: str, log_dir: str, grobid_config: Optional[Dict] = None
) -> Paper:
    """
    :param xml_fpath:
    :param log_dir:
    ...
    with open(xml_fpath, "r") as f:
        try:
            ...
            paper = convert_xml_to_s2orc(
                soup, file_id, year, log_file, grobid_config=grobid_config
            )
            return paper
