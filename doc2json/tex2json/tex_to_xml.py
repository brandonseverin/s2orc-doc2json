...
from s2orc_doc2json.doc2json.utils.latex_util import normalize, latex_to_xml
...
def extract_latex(zip_file: str, latex_dir: str, cleanup=True):
    ...
    assert (
        zip_file.endswith(".gz")
        or zip_file.endswith(".zip")
...
def normalize_latex(
    latex_dir: str, norm_dir: str, norm_log_file: str, cleanup=True
) -> Optional[str]:
    """
...
def norm_latex_to_xml(
    norm_dir: str, xml_dir: str, xml_err_file: str, xml_log_file: str, cleanup=True
) -> Optional[str]:
    """
...
def convert_latex_to_xml(
    ...
    norm_output_dir = normalize_latex(
        latex_output_dir, norm_dir, norm_log_file, cleanup
    )

    # convert to xml
    xml_error_file = os.path.join(log_dir, "xml_error.log")
    xml_log_file = os.path.join(log_dir, "xml_skip.log")
    xml_output_file = norm_latex_to_xml(
        norm_output_dir, xml_dir, xml_error_file, xml_log_file, cleanup
    )
...
def convert_latex_to_s2orc_json(
    latex_zip: str, base_temp_dir: str, cleanup_after: bool = True
) -> str:
    """
