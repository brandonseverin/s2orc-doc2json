...
from s2orc_doc2json.doc2json.tex2json.tex_to_xml import convert_latex_to_s2orc_json
from s2orc_doc2json.doc2json.tex2json.xml_to_json import convert_latex_xml_to_s2orc_json
...
def process_tex_file(
    ...
    # convert to S2ORC
    paper = convert_latex_xml_to_s2orc_json(
        xml_file, log_dir, grobid_config=grobid_config
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", default=None, help="path to the input TEX zip file"
    )
    parser.add_argument(
        "-t", "--temp", default="temp", help="path to a temp dir for partial files"
    )
    parser.add_argument(
        "-o",
        "--output",
