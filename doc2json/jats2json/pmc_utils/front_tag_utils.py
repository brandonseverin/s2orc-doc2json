...
def parse_category_tag(front_tag) -> List[str]:
    ...
    """
    if (
        len(front_tag.find_all("subj-group")) > 1
        or len(front_tag.find_all("subject")) > 1
    ):
...
def parse_date_tag(front_tag) -> Dict:
    ...
    for pub_date in front_tag.find_all("pub-date"):
        ...
        out[pub_date.get("pub-type", "MISSING_PUB_TYPE")] = "-".join(
            [tag.text for tag in [year, month, day] if tag is not None]
        )
        pub_date.decompose()
    for date in front_tag.find_all("date"):
        ...
        day = date.find("day")
        out[date.get("date-type", "MISSING_DATE_TYPE")] = "-".join(
            [tag.text for tag in [year, month, day] if tag is not None]
        )
        date.decompose()
    return out
...
def parse_funding_groups(front_tag) -> List[str]:
    ...
    for tag in front_tag.find_all():
        ...
        if (
            (tag.name == "funding-source" or tag.name == "funding-statement")
            and tag.find("funding-source") is None
            and tag.find("funding-statement") is None
        ):
            out = {
                "name": None,
                "doi": None,
            ...
            if institution_id_tag:
                out["doi"] = institution_id_tag.extract().text.replace(
                    "http://dx.doi.org/", ""
                )
            ...
            funder_id_tag = tag.find(
                "named-content", {"content-type": "funder-identifier"}
            )
            if funder_id_tag:
                out["doi"] = funder_id_tag.extract().text.replace(
                    "http://dx.doi.org/", ""
                )
            ...
            text = tag.text
            if tag.name == "funding-statement" or (
                "fund" in text or "support" in text or "provide" in text
            ):
                out["notes"] = text
            else:
...
def parse_authors(front_tag) -> List[Dict]:
    ...
    for contrib_tag in front_tag.find_all("contrib"):
        ...
        # corresponding tag
        if (contrib_tag.get("corresp") == "yes") or (
            contrib_tag.find("xref", {"ref-type": "corresp"})
        ):
            is_corresp = True
        else:
            is_corresp = False
        ...
        name_info = {
            name_tag.name: name_tag.text
            for name_tag in contrib_tag.find("name").find_all()
        }
        ...
        authors.append(
            {
                ...
                "affiliation_ids": [
                    xref_tag.get("rid")
                    for xref_tag in contrib_tag.find_all("xref", {"ref-type": "aff"})
                ],
...
def parse_affiliations(front_tag) -> List[Dict]:
    ...
    for aff_tag in front_tag.find_all("aff"):
        ...
        for institution_id_tag in aff_tag.find_all("institution-id"):
            id_type_to_id[
                institution_id_tag["institution-id-type"]
            ] = institution_id_tag.text
        ...
        outs.append({"id": aff_id, "other_ids": id_type_to_id, "text": text})
