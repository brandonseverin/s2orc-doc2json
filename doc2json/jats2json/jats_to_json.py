...
from s2orc_doc2json.doc2json.utils.soup_utils import destroy_unimportant_tags_inplace
from s2orc_doc2json.doc2json.jats2json.pmc_utils.front_tag_utils import (
    parse_journal_id_tag,
    parse_journal_name_tag,
    parse_title_tag,
    parse_category_tag,
    parse_date_tag,
    parse_doi_tag,
...
from s2orc_doc2json.doc2json.jats2json.pmc_utils.extract_utils import (
    extract_fig_blobs,
    extract_table_blobs,
    extract_suppl_blobs,
)
from s2orc_doc2json.doc2json.jats2json.pmc_utils.all_tag_utils import (
...
from s2orc_doc2json.doc2json.jats2json.pmc_utils.back_tag_utils import parse_bib_entries

from s2orc_doc2json.doc2json.s2orc import Paper
            ...
            acknowledgements.append(
                {
                    ...
                    "text": par_tag.text,
                    "funding_sources": [
                        fund_tag.text for fund_tag in par_tag.find_all("funding-source")
                    ],
...
def postprocess_front_tags_for_s2orc(init_front_dict: Dict):
    ...
    for a in init_front_dict["authors"]:
        ...
        if a["affiliation_ids"]:
            ...
            affil_text = [
                affil["text"]
                for affil in init_front_dict["affiliations"]
                if affil["id"] == affil_id
            ]
            if affil_text:
                a["affiliation"] = {
                    "laboratory": "",
...
def convert_jats_xml_to_s2orc_json(jats_file: str, log_dir: str):
    ...
    with open(jats_file, "r") as f_in:
        ...
        destroy_unimportant_tags_inplace(
            soup, tags_to_remove=["bold", "italic", "graphic"]
        )
    ...
    for i, (old_table_key, table_blob) in enumerate(sorted(table_blobs.items())):
        ...
        table_text = (
            table_blob["label"]
            + ": "
            + " ".join([c["text"] for c in table_blob["caption"]])
            + "\n"
    ...
    for i, (old_figure_key, figure_blob) in enumerate(sorted(figure_blobs.items())):
        ...
        figure_text = (
            figure_blob["label"]
            + ": "
            + " ".join([c["text"] for c in figure_blob["caption"]])
        )
    ...
    front_dict["abstract"] = convert_paragraphs_to_s2orc(
        front_dict["abstract"], old_key_to_new_key
    )
    ...
    if back_tag is not None:
        ...
        bib_entries = {}
        for i, (old_bib_key, bib_entry) in enumerate(
            sorted(back_dict["bib_entries"].items())
        ):
    ...
    if back_dict and back_dict.get("acknowledgements"):
        back_dict["acknowledgements"] = convert_acks_to_s2orc(
            back_dict["acknowledgements"]
        )
